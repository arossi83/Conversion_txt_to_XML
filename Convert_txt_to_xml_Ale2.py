...
from CheckDuplicatesV2 import CheckDuplicates

if len(sys.argv) < 2:
    sys.exit(0)
elif len(sys.argv)==2:
    datadir=sys.argv[1]
    logfile="/Users/alessandro/Documents/CMS/Phase2/PQC/DBcode/Logs/latest.log"
elif len(sys.argv)==3:
    datadir=sys.argv[1]
    logfile=sys.argv[2]
else:
...
def main(datadir,logdir):
    ...
    try:
            ...
            if (folderName != '__pycache__'):
                ...
                for file in os.listdir(oldPath):
                    ...
                    if fileEx == ("%s_%s_%s_R.xlsx" % (batch[0],batch[1],side[3])):
                        logf.write("Analysis File Right %s found: data will be used\n" % afnR)
                        xlsxName_R = str(fileEx)
                    if fileEx == ("%s_%s_%s.xlsx" % (batch[0],batch[1],side[3])):
                        logf.write("Analysis File Left %s found: data will be used\n" % afn)
                        xlsxName = str(fileEx)
                ...
                if path.exists(oldPath):
                                        ...
                                        if kw in fileCurr:
                                            ...
                                            if rot:
                                                if left:
                                                    ...
                                                    #logf.write("%s - %s - Left - Rot: %s" % (flute,kw,fileCurr))
                                                else:
                                                    outPar.append(("%s - %s - Right - Rot" % (flute,kw)))
                                                    outFile.append(fileCurr)
                                                    #logf.write("%s - %s - Right - Rot: %s" % (flute,kw,fileCurr))
                                            else:
                                                if left:
                                                    outPar.append(("%s - %s - Left - NoRot" % (flute,kw)))
                                                    outFile.append(fileCurr)                                            
                                                    #logf.write("%s - %s - Left - NoRot: %s" % (flute,kw,fileCurr))
                                                else:
                                                    outPar.append(("%s - %s - Right - NoRot" % (flute,kw)))
                                                    outFile.append(fileCurr)                                            
                                                    #logf.write("%s - %s - Right - NoRot: %s" % (flute,kw,fileCurr))
            
                    filteredPar, filteredFile=CheckDuplicates(outPar,outFile,logf)
                    #logf.write(outPar)
                    #logf.write(filteredPar)
                    #logf.write(outFile)
                    #logf.write(filteredFile)
                    ###Process Each Files
                    nL=0
                    nR=0
                    nL_r=0
                    ...
                    for idx,par in enumerate(filteredPar):
                        ...
                        if 'Capacitor' in par and 'flute1' in par:
                            if 'Left' in par:
                                #                        logf.write("%s --> %s" % (par,filteredFile[idx]))
		        ...
		        #Flute4 Poly_Chain
                        if 'Poly_Chain' in par and 'flute4' in par:
                            ...
                            if 'Right' in par:
                                ...
                                nR+=1
                    logf.write("%d Left Measurement processed, plus %d rotated\n" % (nL,nL_r))
                    logf.write("%d Right Measurement processed, plus %d rotated\n" % (nR,nR_r))
                    if nL==24 and nL_r==4 and nR==4 and nR_r==3:
                        check&=True
                    elif nL==14 and nL_r==4 and nR==0 and nR_r==0:
                        check&=True
                    else:
                        check&=False
        ...
        logf.write("something wrong on %s" % oldPath)
        check=False
